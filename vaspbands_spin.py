	...
	rc('ytick', labelsize=16)
        ...
        for n in range(nbands):
            ...
            for k in range(self.nkpt):
                ...
                ys_dn.append(self.evals_dn[k][n])
                
            lcolor = 'red'
            lwidth = 1
            
            plt.plot(xs,ys,color=lcolor, lw=lwidth)
            plt.subplots_adjust(left=0.15,right=0.95,top=0.95,bottom=0.10)

	    lcolor = 'blue'
            plt.plot(xs,ys_dn,color=lcolor, lw=lwidth, ls='--')
            plt.subplots_adjust(left=0.15,right=0.95,top=0.95,bottom=0.10)       
...
#	    print line
            ...
            for m in range(self.nbands):
                ...
                ev = float(sline[2]) - self.fermi
                self.evals_dn[-1].append(ev)
